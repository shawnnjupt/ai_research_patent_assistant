...
class PaperRecord:
    ...
    def to_dict(self):
            return {
                ...
                "first_submitted_date": self.paper.first_submitted_date.strftime("%Y-%m-%d")
                    if self.paper.first_submitted_date else None,
                "first_announced_date": self.paper.first_announced_date.strftime("%Y-%m-%d")
                    if self.paper.first_announced_date else None,
                "comments": self.paper.comments,
                "reason": self.comment
            }
...
class PaperDatabase:
    ...
    def __enter__(self):
        return self  # 返回自己，供 with ... as db 使用
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        return False  # 返回 True 表示异常已被处理，否则返回 False
...
""".lstrip()
    ...
    def to_json(self, output_dir="./output_json", filename_format="%Y-%m-%d", metadata=None):
        """
        将每天的论文导出为结构化 JSON 文件
        """
        output_dir = Path(output_dir)
        output_dir.mkdir(exist_ok=True, parents=True)

        global_metadata = {
            "category_whitelist": list(self.categories_whitelist),
            "category_blacklist": list(self.categories_blacklist),
        }
        if metadata:
        ...
        for i in range(self.date_range_days):
            ...
            data = {
                ...
                "chosen": [record.to_dict() for record in chosen_records],
                "filtered": [record.to_dict() for record in filtered_records]
            }

            with open(output_dir / f"{current_filename}.json", "w", encoding="utf-8") as f:
                json.dump(data, f, ensure_ascii=False, indent=2, default=lambda o: o.strftime("%Y-%m-%d") if isinstance(o, (datetime, date)) else None)

            self.console.log(
                f"[bold green]Output {current_filename}.json completed. {len(chosen_records)} papers chosen, {len(filtered_records)} papers filtered"
            )
