...
export default function PatentSearchResults({ patents, isLoading = false }: PatentSearchResultsProps) {
  if (isLoading) {
    return (
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h3 className="text-lg font-medium">找到 {patents.length} 项相关专利</h3>
      ...
      <div className="space-y-4">
        {patents.map((patent) => (
          <Card key={patent.id} className="w-full hover:shadow-md transition-shadow">
            <CardHeader className="pb-2">
              ...
              <div className="flex flex-wrap gap-2 text-sm text-muted-foreground">
                ...
                <span>申请日: {patent.applicationDate}</span>
                <span>公开日: {patent.publicationDate}</span>
              </div>
            </CardHeader>
            <CardContent>
              <p className="text-sm text-muted-foreground line-clamp-3">{patent.abstract}</p>
              <div className="flex flex-wrap gap-1 mt-3">
                <Badge variant="outline">{patent.patentType}</Badge>
